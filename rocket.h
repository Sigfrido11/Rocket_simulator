...
public:
  ...
  /* explicit Rocket(double x, double Up_Ar, double Lat_Ar, double spm, double
     st, int num_stage, std::vector<double> lpm, std::vector<double> lt) :
 ...
 */
  ...
  Rocket(double x, double Up_Ar, double Lat_Ar, double spm, double scm,
         int num_stage, std::vector<double> lpm, std::vector<double> lcm)
      : total_mass_{x + std::accumulate(lpm.begin(), lpm.end(), 0.) +
                    std::accumulate(lcm.begin(), lcm.end(), 0.) + scm +
                    spm}, // x in realtà solo navicella
        lateral_area_{Lat_Ar}, upper_area_{Up_Ar}, mass_solid_prop_{spm},
        mass_solid_cont_{scm}, total_stage_{num_stage},
        current_stage_{num_stage}, mass_liq_prop_{lpm}, mass_liq_cont_{lcm} {}
  ...
  public:
    ...
    double delta_m(double, double) const;
