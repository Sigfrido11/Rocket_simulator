...
Vec const Rocket::thrust(double p_ext, double time, bool is_orbiting) const {
  ...
  double const y{engs[0] * n_sol_eng_ + engl[0] * n_liq_eng_[0]};
  return {z, y};
}
...
Vec const Rocket::Ad_engine::eng_force(std::vector<double> par,
  ...
  if (!is_orbiting && !released_) {
    ...
    double const fac4{1 - std::pow((p_ext / p_0_), (sim::cost::gamma_ -
                                                    1 / sim::cost::gamma_))};
    double const force{fac1 * std::sqrt(fac2 * std::pow(fac3, exp) * fac4)};
    return {force * std::sin(theta), force * std::cos(theta)};
  } else {
    return {0., 0.};
  }
...
double Rocket::Ad_engine::delta_m(double time, bool is_orbiting) const {
  if (!is_orbiting && !released_) {
    ...
    double const fac2{std::sqrt(sim::cost::gamma_ / (sim::cost::R_ * t_0_))};
    double const fac3{2 / (sim::cost::gamma_ + 1)};
...
inline double improve_theta(std::string name_f, double pos, double orbital_h,
...
Vec const total_force(double rho, double theta, double total_mass, double pos,
                      double upper_area, double lateral_area, Vec velocity,
                      Vec eng) {
  Vec const centrip {centripetal(total_mass, pos, velocity[1])};
  Vec const gra {g_force(pos, total_mass, theta)};
  Vec const drag_f {drag(rho, pos, theta, upper_area, lateral_area, velocity)};
  double const z{eng[0] - gra[0] - drag_f[0]};
  double const y{eng[1] + centrip[1] - gra[1] - drag_f[1]};
  return {z, y};
}
};  // namespace rocket