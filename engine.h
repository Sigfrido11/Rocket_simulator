...
public:
    ...
    virtual Vec eng_force(double pa, double time, double theta,
                          bool is_orbiting) = 0;
...
public:
    ...
    Vec eng_force(double pa, double time, double theta,
                  bool is_orbiting) override;
...
class Ad_sol_engine final : public Engine {
    ...
    double V_c_{0.5};        // Combustion chamber volume [m^3]
...
private:
    ...
    //compute pe
    double compute_exit_pressure(double Pc) const;
...
private:
    ...
    double compute_exit_pressure();

    // Solve for exit Mach number given area ratio and gamma
    static double isentropic_area_ratio(double M, double gamma);

    // Solve for exit Mach number given area ratio and gamma
    static double exit_mach(double areaRatio, double gamma);
