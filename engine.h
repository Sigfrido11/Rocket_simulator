...
public:
    ...
    // time         -> time step [s]
    // is_orbiting  -> whether vehicle is in orbital phase (may affect logic)
    ...
    virtual Vec eng_force(double pa, double pe,
...
public:
    Base_engine() = default;
    Base_engine(double isp, double cm, double p0);

    double delta_m(double time, bool is_orbiting) const override;

    Vec eng_force(double pa, double pe,
...
public:
    ...
    Vec eng_force(eng_par const& par,
                  bool is_orbiting) const override;
