...
int main() {
  ...
  try {
    {  // aggiungo uno scope in modo da cancellare poi tutte le
      ...
      std::cout
          << "let's build the engine for your rocket"
          << "\n"
      ...
      switch (ans_eng) {
        ...
        case 'b':
          ...
          base_eng =
              rocket::Base_engine{base_eng_data.isp, base_eng_data.cm,
      ...
      switch (ans_rocket) {
        ...
        case 'm':
          ...
          std::fill_n(l_c_m.begin(), rocket_data.stage_num,
    ...
    sim::Air_var air;
    std::cout << "at which altitude do you want to orbit? "
              << "\n";
    double orbital_h;
    std::cin >> orbital_h;
    ...
    std::vector<sf::Drawable *> drawables{
        &map,     &ground,    &inner_atm, &rocket1, &earth,
        &rocket2, &rocket3,   &altitude,  &angle,   &speed,
        &stage,   &fuel_left, &time};
...
*/
    ...
    std::cerr << "known exception detected: " << e.what() << std::endl;
  } catch (...) {
