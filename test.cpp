...
int main() {
  ...
  while (i < 400) {
    ...
    bool orbiting =rocket::is_orbiting(rocket.get_pos()[0], rocket.get_velocity()[1]);

    rocket.set_state(file_name, orbital_h, time, orbiting, start_pos);

    air.set_state(rocket.get_pos()[0]);

    Vec eng_force = rocket.thrust(air.p_, time, orbiting);

    force = rocket::total_force(air.rho_, rocket.get_theta(), rocket.get_mass(),
                                rocket.get_pos()[0], time, rocket.get_up_ar(),
                                rocket.get_lat_ar(), orbiting,
